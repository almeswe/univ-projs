...
type TArrayCreationKind = (REVERSED_A, SORTED_A, RANDOM_A);
...
function SelectionSort(data : TIntArray; out comps,perms : uint) : TIntArray;
...
begin
  for i := 0 to len(data)-1 do begin
    ...
    for j := i+1 to len(data)-1 do begin
      if LessThan(data[j], data[i], comps) then begin
        swaps := true;
        swap(data[i], data[j], perms);
      end;
    end;
    if not swaps then
      break;
